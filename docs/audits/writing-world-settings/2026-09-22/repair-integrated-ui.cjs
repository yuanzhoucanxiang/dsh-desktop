...
fixture=fixture.slice(0,cut)+String.raw`
  ...
  await evaluate('(()=>{window.__confirmation="";window.confirm=t=>{window.__confirmation=t;return false};return true})()');
  await click(evaluate,'导入旧位置草稿');
  assert.ok((await evaluate('window.__confirmation')).includes(oldLoc));assert.ok((await evaluate('window.__confirmation')).includes(project));
  assert.equal(draftHost.listCheckpoints(project).some(c=>c.text==='移动前未发送草稿'),false);
  await evaluate('(()=>{window.confirm=t=>{window.__confirmation=t;return true};return true})()');
  ...
  observations.push({id:'D04-manual',status:'PASS',issue:'Unrelated import: source/target shown, cancellation preserves target, explicit confirmation imports copies'});
  for(let n=0;n<6;n++){draftHost.writeCheckpoint(project+String.fromCharCode(0)+'world','late'+n,{baseRev:0,text:JSON.stringify({version:1,drafts:[{id:'late-draft-'+n,title:'延迟恢复'+n,conclusion:'延迟副本正文'+n,dirty:true}]})});await sleep(5)}
  await click(evaluate,'查找可恢复编辑');await waitFor('Array.from(document.querySelectorAll("[data-world-panel] button")).some(b=>b.parentElement.textContent.includes("延迟恢复0"))');
  await evaluate('(()=>{window.__originalFetch=window.fetch;window.__held=null;window.fetch=(url,opts)=>{const u=new URL(String(url),location.href);if(u.searchParams.get("route")==="world-draft"&&u.searchParams.get("window")==="late0"){return new Promise((resolve,reject)=>{window.__held=()=>resolve(window.__originalFetch(url,opts));window.__failRecovery=()=>reject(new Error("fixture-network-down"))})}return window.__originalFetch(url,opts)};return true})()');
  const lateClick='Array.from(document.querySelectorAll("[data-world-panel] button")).find(b=>b.textContent==="恢复这份编辑"&&b.parentElement.textContent.includes("延迟恢复0")).click()';
  await evaluate(lateClick);await waitFor('!!window.__held');
  await field(evaluate,'conclusion','等待期间的新编辑必须保留');
  await evaluate('(()=>{window.__failRecovery();window.__held=null;return true})()');
  await waitFor('document.querySelector("[data-world-journal]").textContent.includes("恢复失败")');
  assert.equal(await evaluate('document.querySelector("[data-world-field=conclusion]").value'),'等待期间的新编辑必须保留');
  observations.push({id:'V7-network-retry',status:'PASS',issue:'Lazy recovery network failure is visible, preserves local text, and permits retry'});
  await evaluate(lateClick);await waitFor('!!window.__held');
  await evaluate('(()=>{window.__held();return true})()');
  await waitFor('document.querySelector("[data-world-field=conclusion]").value==="延迟副本正文0"');
  assert.ok(await evaluate('(()=>{const k=Object.keys(sessionStorage).find(k=>k.startsWith("dsh-world-drafts-v1:"));return JSON.parse(sessionStorage.getItem(k)).drafts.some(d=>d.conclusion==="等待期间的新编辑必须保留")})()'));
  observations.push({id:'V7-late-edit',status:'PASS',issue:'Beyond four prefetched snapshots: actual delayed fetch preserves edits typed while waiting'});
  await evaluate('(()=>{window.__held=null;return true})()');await evaluate(lateClick);await waitFor('!!window.__held');
  await button('收起备忘');await waitFor('!document.querySelector("[data-world-panel]")');
  const cacheBefore=await evaluate('JSON.stringify(Object.fromEntries(Object.keys(sessionStorage).filter(k=>k.startsWith("dsh-world-drafts-v1:")).map(k=>[k,sessionStorage.getItem(k)])))');
  await evaluate('(()=>{window.__held();window.fetch=window.__originalFetch;return true})()');await sleep(300);
  assert.equal(await evaluate('JSON.stringify(Object.fromEntries(Object.keys(sessionStorage).filter(k=>k.startsWith("dsh-world-drafts-v1:")).map(k=>[k,sessionStorage.getItem(k)])))'),cacheBefore);
  observations.push({id:'V7-unmount',status:'PASS',issue:'Closing panel during delayed read cancels adoption; old callback does not mutate saved local edits'});
  const strongOld=path.join(root,'真实迁移来源'),migrated=strongOld+'-已移动';
  fs.mkdirSync(path.join(strongOld,'draft/novel'),{recursive:true});fs.mkdirSync(path.join(strongOld,'state'));
  fs.writeFileSync(path.join(strongOld,'project.md'),'真实迁移验证');fs.writeFileSync(path.join(strongOld,'draft/novel/迁移验证.md'),'迁移稿件');
  ...
  await waitFor('document.querySelector("[data-world-notice]").textContent.includes("旧位置草稿已导入")');
  assert.ok((await evaluate('window.__confirmation')).includes(strongOld));assert.ok((await evaluate('window.__confirmation')).includes(migrated));
  assert.ok(draftHost.listCheckpoints(migrated).some(c=>c.text==='真实迁移未发送文字'));
  observations.push({id:'D04-real-move',status:'PASS',issue:'Entire project directory actually renamed; memory identity finds old location; UI imports old chat checkpoint into moved project'});
  win.showInactive();await sleep(250);fs.writeFileSync(path.join(repo,'docs/audits/writing-world-settings/2026-09-22/repair-integrated-ui.png'),(await win.webContents.capturePage()).toPNG());
  fs.writeFileSync(path.join(repo,'docs/audits/writing-world-settings/2026-09-22/repair-integrated-ui-results.json'),JSON.stringify({temp,observations},null,2));console.log('AUDIT_UI_COMPLETE',JSON.stringify(observations));
