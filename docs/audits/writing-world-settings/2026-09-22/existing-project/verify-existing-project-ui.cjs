...
rep("  fs.mkdirSync(path.dirname(doc), { recursive: true })","  fs.mkdirSync(root, { recursive: true }); fs.cpSync('E:/BaiduNetdiskDownload/剧本创作',project,{recursive:true})")
...
fixture=fixture.slice(0,cut)+String.raw`
  ...
  for(const rel of fs.readdirSync('E:/BaiduNetdiskDownload/剧本创作',{recursive:true}).filter(x=>x.endsWith('.md'))){const a=fs.readFileSync(path.join('E:/BaiduNetdiskDownload/剧本创作',rel)),b=fs.readFileSync(path.join(project,rel));assert.deepEqual(a,b);hashes.push({path:rel,sha256:crypto.createHash('sha256').update(a).digest('hex')})}
  ...
  fs.writeFileSync(path.join(repo,'existing-preview.json'),JSON.stringify({pid:process.pid,base:temp,home,project,port,keptRunning:process.env.WM_KEEP_PREVIEW==='1',results,hashes},null,2));
