// Round 4: production HTTP/storage + controlled independent-process schedules.
...
const temp=fs.mkdtempSync(path.join(os.tmpdir(),'wm-review4-'))
...
try{
 ...
 await check('W01 moving a replacement live lock opens acquisition gap','REPRODUCED',async()=>{
   ...
   const a=child(p,'reclaimer','reclaimerA',1,initial.etag);let b,c
   try{
     await ready(path.join(p,'reclaimerA.before.ready'))
     ...
     fs.writeFileSync(path.join(p,'reclaimerA.before.release'),'1')
     await ready(path.join(p,'reclaimerA.after.ready'))
     assert.equal(fs.existsSync(lock),false)
     c=child(p,'holder','holderC',1,initial.etag);await ready(path.join(p,'holderC.ready'))
     assert.equal(b.c.exitCode,null);assert.equal(c.c.exitCode,null)
     fs.writeFileSync(path.join(p,'reclaimerA.after.release'),'1');await sleep(100)
     const ownerAfterRestore=fs.existsSync(lock)?fs.readFileSync(lock,'utf8'):null
     fs.writeFileSync(path.join(p,'holderC.release'),'1');const rc=await c.done;assert.equal(rc.code,0,rc.out+rc.err)
     ...
     const final=memory.readMemory(p).memory;assert.equal(final.items.some(i=>i.text==='holderC'),false)
     return {a:ra,b:rb,c:rc,ownerAfterRestore,finalTexts:final.items.map(i=>i.text)}
   }finally{for(const n of ['reclaimerA.before','reclaimerA.after','holderB','holderC'])fs.writeFileSync(path.join(p,n+'.release'),'1');await a.done;if(b)await b.done;if(c)await c.done}
 })
 await check('W03 deletion resets revision and reaccepts stale baseRev zero','REPRODUCED',async()=>{
   ...
   assert.throws(()=>drafts.writeCheckpoint(project,'clear-bucket',{text:'AFTER_CLEAR',baseRev:cleared.rev}),e=>e.status===409)
   const stale=drafts.writeCheckpoint(project,'clear-bucket',{text:'DELAYED_BEFORE_FIRST_SAVE',baseRev:0})
   assert.equal(stale.checkpoint.rev,1)
   return {clearReturnedRev:cleared.rev,nextCorrectRevRejected:409,oldBaseZeroAccepted:true,storedText:stale.checkpoint.text}
 })
}finally{await new Promise(r=>server.close(r));fs.writeFileSync(path.join(temp,'results.json'),JSON.stringify({reviewedCommit:'606f21f',results},null,2));console.log('EVIDENCE',temp)}
