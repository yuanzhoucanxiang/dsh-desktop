...
- 一键入口：`npm run verify:writing-all`（18 步：架构 → 构建 → host 回归 → 适配 → 备忘 → 上下文 →
  包/冷启动/升级 → 复核负向探针 → 原 32 条正向基线 → 主题 UI 冒烟 → 单测 → 三套 Electron E2E → 真包验收）
...
| `verify:writing-packaged` | **真包**冷启动 + 首启种子 + 主题开启/恢复（7 项） | 需先构建；`WM_PKG` 指向解包目录 |
...
| A02 | 打包后离线加载 | PASS | `verify:writing-package --package <解包目录>`：app.asar 内 9 个主进程依赖齐全、**包内无仓库绝对路径**、extraResources（插件 15 文件 + runtime.tar.gz + runtime-marker + builtin-plugins）完整；`verify:writing-packaged`：真包在四路径隔离下 `SMOKE_OK` 且 profile 由应用自行同步（15 文件哈希一致） | 未构造受限网络环境（模型在线能力不计入本项） |
...
| H01 | 连点发送/设置；两窗口首次关联 | PASS | `verify:writing-adapter` H01/H02：两窗口同作品→只建 1 个会话，后到窗口采用先到者绑定；连点走进程内共用 Promise | 两窗口为两个 adapter 实例 + 真 host 协调协议，非两个真实 Electron 窗口 |
...
| D04 | 两窗口各自编辑/恢复 | PASS | 候选条带窗口与时间；**复核 B06 探针**：采用前先把当前草稿存成可恢复副本（`-before-adopt-*` 桶，已实测能取回）、引用按候选显式置空、失败不切换 | UI 层为渲染器探针 + 真 host，非两个真实窗口 |
...
| M03 | 项目 A/B、同名目录、大小写与移动 | PASS | host 规范化项目身份分桶（大小写/分隔符/尾斜杠归一，`bucketOf` 单测）；`verify:writing` 项目隔离用例 | **作品目录移动后**的备忘/协调记录迁移未做 |
...
| C03 | 关闭自动参考 / 切项目 / 压缩后续聊 | PASS | C05 关闭后正文照发且不含备忘；chat E2E 实测「关掉参考即不带」；项目隔离同 M03；面板用**同一选择函数**显示实际采用/省略与来源，并支持逐条排除与"优先固定" | 跨轮去重未做（方案要求先验证 compaction API） |
| C04 | 引用含"忽略规则"等指令文本 | PASS | 引用块有明确数据边界与来源标注；`reference.js` 身份结构断言（path/revision/选区/指纹）+ 缺身份时**保守比较完整快照** | — |
| P01 | 空白 home / 已有 v0.1.38 home | PASS | `verify:writing-package`：空环境冷启动 6/6（启动前证明 profile 不存在→应用自行同步→15 文件哈希一致）；升级路径 5/5（旧设置/备忘字节不变、旧文件保留不删、坏 JSON 保留原件） | 升级 fixture 为脱敏构造，非真实用户数据副本；"作者能打开并继续保存"由 E2E 覆盖，未在升级 fixture 上再走一遍 UI |
