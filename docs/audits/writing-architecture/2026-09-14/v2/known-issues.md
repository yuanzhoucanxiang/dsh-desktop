...
> 2026-09-19 更新：下文为历史交回边界。真实模型三场景和同一 host 的两真实窗口已复测，安装器已在 9 月 15 日构建发布，但 NSIS 隔离安装、macOS 实机仍未验收。
> 新发现的候选来源问题已在工作区修复，尚未发布。最新状态统一见 [2026-09-19 报告](../../2026-09-19/review.md)。

> 本轮（复核退回后）已修：新包启动失败、旧消息被当受理证据、采用候选丢原稿、恢复另建会话、
> 备忘失败仍照发、同步越界、引用指纹丢失、矩阵缺 A01–A03。下面保留的是仍未做/仍有边界的部分。

按"影响 × 是否可绕过"排序。未验证项不能据此排除稿件/设置风险；当时的未验证项见 `acceptance.md` 末表。
