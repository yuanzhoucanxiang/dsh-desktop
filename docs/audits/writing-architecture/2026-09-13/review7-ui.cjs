// Round 7: real production React/HTTP, stable native-shaped input, delayed transport.
...
fixture=fixture.slice(0,cut)+String.raw`
  ...
  record('V01 recovery 413 produces visible save error',oversize.alerts.length?'PASS':'REPRODUCED',{...oversize,disk:draftHost.readCheckpoint(project,windowId).text})
  ...
  record('Shorter draft saves after 413','PASS',{})
  ...
  record('V01 failed local resolution exposes current save failure',resolveFailure.alerts.some(x=>x.includes('fixture-write-unavailable'))?'PASS':'REPRODUCED',{...resolveFailure,disk:draftHost.readCheckpoint(project,windowId).text})
  await input('.dshWmChatInput','AFTER_RESOLVE_FAILURE');await waitFor('testChat.responses.length===4')
  ...
  record('V01 recovery network rejection is observable to author',net.alerts.length?'PASS':'REPRODUCED',{...net,disk:draftHost.readCheckpoint(project,windowId).text})
  fs.writeFileSync(path.join(temp,'review7-ui.json'),JSON.stringify({reviewedImplementation:'2c3ab6e',head:'a58b3c7',results,consoleErrors:errors},null,2));console.log('REVIEW7_UI_EVIDENCE',temp)
