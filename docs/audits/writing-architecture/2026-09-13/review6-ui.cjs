// Round 6: real production React/HTTP, stable native-shaped input, delayed transport.
...
fixture=fixture.slice(0,cut)+String.raw`
  ...
  await waitFor('!!testChat.releaseWrite')
  draftHost.writeCheckpoint(project,windowId,{text:'BOUND_REMOTE',baseRev:draftHost.readCheckpoint(project,windowId).rev})
  await evaluate('testChat.releaseWrite()')
  await waitFor("document.body.innerText.includes('采用远端')")
  await button('采用远端');await waitFor('testChat.responses.length===2');await sleep(100)
  ...
  record('S01 bound remote selection must update actual native input',boundVisible==='BOUND_REMOTE'?'PASS':'REPRODUCED',{visible:boundVisible,disk:draftHost.readCheckpoint(project,windowId).text})
  await evaluate("document.querySelector('.dshWmSend').click()")
  await waitFor('testChat.calls.length===1')
  const sent=await evaluate('testChat.calls[0].content')
  record('S01 send after remote choice',JSON.stringify(sent).includes('BOUND_LOCAL')?'REPRODUCED':'PASS',{sent})
  ...
  record('S02 recovery-flush conflict presents recovery actions',pendingBody.includes('采用远端')?'PASS':'REPRODUCED',{hasConflictAction:pendingBody.includes('采用远端'),visible:await evaluate("document.querySelector('.dshWmChatInput').value"),disk:draftHost.readCheckpoint(project,windowId).text})
  ...
  await waitFor("document.body.innerText.includes('采用远端')")
  await button('采用远端');await waitFor('testChat.responses.length===2');await sleep(100)
  const afterFailedRemote=await evaluate("document.querySelector('.dshWmChatInput').value")
  assert.equal((await evaluate('testChat.responses'))[1].status,409)
  record('S03 missing remote snapshot is not an empty draft',afterFailedRemote==='LOCAL_MUST_SURVIVE_FAILED_READ'?'PASS':'REPRODUCED',{visible:afterFailedRemote,disk:draftHost.readCheckpoint(project,windowId).text,statuses:(await evaluate('testChat.responses')).map(r=>r.status)})
  fs.writeFileSync(path.join(temp,'review6-ui.json'),JSON.stringify({reviewedImplementation:'985851e',head:'4caaa10',results,consoleErrors:errors},null,2));console.log('REVIEW6_UI_EVIDENCE',temp)
