'use strict'

/**
 * 改动审阅 git 层的单元测试（普通 node 跑，秒级）：
 *   node lib/git-review.test.js       → 打印每项 PASS/FAIL，exit 0 = 通过
 * 在系统临时目录里建真仓库、跑真 git，测完删干净；不碰用户仓库、不起 Electron。
 */

const { execFileSync } = require('node:child_process')
const fs = require('node:fs')
const os = require('node:os')
const path = require('node:path')
const { createGitReview } = require('./git-review')

const failures = []
function check(name, ok, detail) {
  if (!ok) failures.push(name)
  console.log(`${ok ? 'PASS' : 'FAIL'} ${name}${detail ? '  [' + detail + ']' : ''}`)
}

const repo = fs.mkdtempSync(path.join(os.tmpdir(), 'dsh-gitrev-'))
const g = createGitReview(() => repo)
const sh = (args) => execFileSync('git', args, { cwd: repo, encoding: 'utf8', windowsHide: true })
const write = (p, s) => fs.writeFileSync(path.join(repo, p), s)
const readFile = (p) => fs.readFileSync(path.join(repo, p), 'utf8')

try {
  /* ── 准备：一个有 10 行的文件，先提交打底 ─────────────────────────────── */
  sh(['init', '-q'])
  sh(['config', 'user.email', 't@t'])
  sh(['config', 'user.name', 'tester'])
  sh(['config', 'commit.gpgsign', 'false'])
  const base = Array.from({ length: 12 }, (_, i) => `line ${i + 1}`).join('\n') + '\n'
  write('a.txt', base)
  sh(['add', 'a.txt'])
  sh(['commit', '-q', '-m', 'base'])

  check('isRepo() 认得仓库', g.isRepo() === true)
  check('非仓库目录返回 isGit:false', createGitReview(() => os.tmpdir()).changes().isGit === false)

  /* ── 造两处相距很远的改动 → 应该切成 2 个 hunk ─────────────────────────── */
  const edited = base.replace('line 1', 'LINE ONE').replace('line 12', 'LINE TWELVE')
  write('a.txt', edited)
  write('new.txt', 'brand new\n') // 未跟踪文件

  let c = g.changes()
  check('changes() 列出两个文件', c.files.length === 2, c.files.map((f) => f.path).join(','))
  const a = c.files.find((f) => f.path === 'a.txt')
  const n = c.files.find((f) => f.path === 'new.txt')
  check('未跟踪文件被标记', !!n && n.untracked === true)
  check('a.txt 切出 2 个未暂存 hunk', a.hunksUnstaged === 2, `hunks=${a.hunksUnstaged}`)
  check('a.txt 还没有已暂存内容', a.hunksStaged === 0)
  check('changes() 带分支名', !!c.branch, c.branch)

  /* ── 逐 hunk 暂存：只暂存第 0 块 ───────────────────────────────────────── */
  const p0 = g.hunkPatch('a.txt', 0, false)
  check('hunkPatch 生成合法补丁头', p0.ok && /^diff --git /.test(p0.patch) && p0.patch.includes('@@'),
    p0.ok ? p0.patch.split('\n')[0] : p0.error)
  check('hunkPatch 越界会报错', g.hunkPatch('a.txt', 9, false).ok === false)

  check('stage(第0块) 成功', g.stage('a.txt', 0).ok === true)
  c = g.changes()
  const a2 = c.files.find((f) => f.path === 'a.txt')
  check('暂存后：已暂存 1 块 / 未暂存 1 块',
    a2.hunksStaged === 1 && a2.hunksUnstaged === 1,
    `staged=${a2.hunksStaged} unstaged=${a2.hunksUnstaged}`)
  check('暂存的是第 1 行那块', a2.diffStaged.includes('LINE ONE') && !a2.diffStaged.includes('LINE TWELVE'))
  check('工作区文件未被改动', readFile('a.txt') === edited)
  check('stagedCount 统计正确', c.stagedCount === 1, String(c.stagedCount))

  /* ── 取消暂存（逐块 → 回到全未暂存） ──────────────────────────────────── */
  check('unstage(第0块) 成功', g.unstage('a.txt', 0).ok === true)
  const a3 = g.changes().files.find((f) => f.path === 'a.txt')
  check('取消暂存后：已暂存 0 块 / 未暂存 2 块',
    a3.hunksStaged === 0 && a3.hunksUnstaged === 2,
    `staged=${a3.hunksStaged} unstaged=${a3.hunksUnstaged}`)
  check('取消暂存不动工作区', readFile('a.txt') === edited)

  /* ── 逐 hunk 丢弃：只丢第 1 块（末尾那处），第 0 块必须留着 ───────────── */
  check('revertHunk(第1块) 成功', g.revertHunk('a.txt', 1).ok === true)
  const after = readFile('a.txt')
  check('第 1 块被丢弃（line 12 复原）', after.includes('line 12') && !after.includes('LINE TWELVE'))
  check('第 0 块仍在（LINE ONE 保留）', after.includes('LINE ONE'), '这是"逐块"的关键：不能把整文件都还原了')

  /* ── 提交：空暂存区要拒绝，提交后工作区干净 ────────────────────────────── */
  check('空暂存区拒绝提交', g.commit('nope').ok === false)
  check('空提交信息被拒绝', g.commit('   ').ok === false)
  check('stage 整文件成功', g.stage('a.txt', null).ok === true)
  const cm = g.commit('feat: 逐块暂存后提交')
  check('commit 成功并返回短 hash', cm.ok === true && /^[0-9a-f]{6,}$/.test(cm.hash || ''), cm.ok ? cm.hash : cm.error)
  check('提交后 a.txt 不再出现在改动里',
    !g.changes().files.some((f) => f.path === 'a.txt'),
    g.changes().files.map((f) => f.path).join(','))

  /* ── 未跟踪文件：暂存 → 取消暂存 → 丢弃（删除） ────────────────────────── */
  check('stage 未跟踪文件成功', g.stage('new.txt', null).ok === true)
  check('暂存后仍被列出', g.changes().files.some((f) => f.path === 'new.txt'))
  check('unstage 未跟踪文件成功', g.unstage('new.txt', null).ok === true)
  check('revertFile(未跟踪) 删掉文件', g.revertFile('new.txt', true).ok === true
    && !fs.existsSync(path.join(repo, 'new.txt')))
  /* 回归：外壳 IPC 层传进来的是绝对路径（workspacePath 已解析），
     path.join(cwd, abs) 会拼出畸形路径导致删除静默失效（revertFile 修复）。 */
  fs.writeFileSync(path.join(repo, 'abs-new.txt'), 'x')
  check('revertFile(绝对路径·未跟踪) 删掉文件',
    g.revertFile(path.join(repo, 'abs-new.txt'), true).ok === true
    && !fs.existsSync(path.join(repo, 'abs-new.txt')))

  /* ── 无远端时推送要给出人话错误 ────────────────────────────────────────── */
  const ps = g.push()
  check('没有远端时 push 明确报错', ps.ok === false && /远端|remote/.test(ps.error || ''), ps.error)

  /* ── 已删除文件也要能审阅/暂存 ─────────────────────────────────────────── */
  fs.rmSync(path.join(repo, 'a.txt'))
  const del = g.changes().files.find((f) => f.path === 'a.txt')
  check('删除也算改动并有 diff', !!del && del.diffUnstaged.includes('@@'), del ? del.status : 'missing')
  check('stage 删除操作成功', g.stage('a.txt', null).ok === true)
  check('删除进入暂存区', (g.changes().files.find((f) => f.path === 'a.txt') || {}).hunksStaged >= 1)

  /* ── 批量：stageAll / revertAll ─────────────────────────────────────────── */
  // 先清干净暂存区，制造混合现场
  execFileSync('git', ['reset', '-q', 'HEAD'], { cwd: repo, windowsHide: true })
  fs.writeFileSync(path.join(repo, 'b.txt'), 'b1\n')
  fs.writeFileSync(path.join(repo, 'new.txt'), 'n\n')
  fs.writeFileSync(path.join(repo, 'keep.txt'), 'k\n')
  const sa = g.stageAll()
  check('stageAll 成功', sa.ok === true, sa.error)
  const afterAll = g.changes()
  check('stageAll 后暂存区非空', afterAll.stagedCount >= 2, String(afterAll.stagedCount))
  check('stageAll 后无未暂存残留',
    afterAll.files.every((f) => !f.untracked && !f.diffUnstaged),
    afterAll.files.map((f) => `${f.path}:${f.untracked ? 'U' : ''}${f.diffUnstaged ? 'M' : ''}`).join(','))
  const ra = g.revertAll()
  check('revertAll 成功', ra.ok === true, ra.error || ra.note)
  const afterRa = g.changes()
  check('revertAll 后工作区干净', afterRa.files.length === 0,
    afterRa.files.map((f) => f.path).join(','))
  check('revertAll 删除了未跟踪文件', !fs.existsSync(path.join(repo, 'b.txt'))
    && !fs.existsSync(path.join(repo, 'new.txt')))

  /* ── G1：中文文件名不得被八进制转义（core.quotepath 默认开）─────────────── */
  // 本项目默认库根就是中文路径，写作模式投影生成的正是 bible/世界观整理.md；
  // 旧写法只脱引号不解码，导致中文文件在面板里 diff 空白、块数 0、暂存按钮不出现。
  fs.mkdirSync(path.join(repo, 'bible'), { recursive: true })
  write('bible/plain.md', 'line1\nline2\n')
  write('bible/世界观整理.md', 'line1\nline2\n')
  sh(['add', '--', 'bible'])
  sh(['commit', '-q', '-m', 'add bible'])
  write('bible/plain.md', 'line1\nline2-CHANGED\n')
  write('bible/世界观整理.md', 'line1\nline2-CHANGED\n')
  write('bible/新章节.md', '新章节正文\n') // 已跟踪目录里的中文未跟踪文件
  fs.mkdirSync(path.join(repo, '新作品', 'draft'), { recursive: true })
  write('新作品/project.md', '# 新作品\n')
  write('新作品/draft/第一章.md', '正文\n')

  const cc = g.changes()
  const cn = cc.files.find((f) => f.path === 'bible/世界观整理.md')
  const ascii = cc.files.find((f) => f.path === 'bible/plain.md')
  check('G1 中文文件名以真实路径出现（不再八进制转义）', !!cn, cc.files.map((f) => f.path).join(','))
  check('G1 中文文件 diff 与块数与 ASCII 对照组一致',
    !!cn && !!ascii && cn.hunksUnstaged === 1 && ascii.hunksUnstaged === 1 && cn.diffUnstaged.includes('CHANGED'),
    cn ? `hunks=${cn.hunksUnstaged} diffLen=${cn.diffUnstaged.length}` : 'missing')
  check('G1 中文文件可以逐块暂存', !!cn && g.stage('bible/世界观整理.md', 0).ok === true)
  check('G1 暂存后中文文件进入已暂存',
    (g.changes().files.find((f) => f.path === 'bible/世界观整理.md') || {}).hunksStaged === 1)
  check('G1 中文文件可以取消暂存', g.unstage('bible/世界观整理.md', null).ok === true)

  /* ── G4：新目录不得折叠成一行 dir/（否则目录内新文件全部不可见）────── */
  check('G4 没有折叠的目录行', !cc.files.some((f) => f.path.endsWith('/')),
    cc.files.filter((f) => f.path.endsWith('/')).map((f) => f.path).join(','))
  check('G4 新目录里的文件逐个可见',
    cc.files.some((f) => f.path === '新作品/project.md') && cc.files.some((f) => f.path === '新作品/draft/第一章.md'))

  /* ── G2：删除必须真删；不得对不存在的路径/目录报成功 ─────────────── */
  const cnUn = cc.files.find((f) => f.path === 'bible/新章节.md')
  check('G2 中文未跟踪文件逐文件列出且标 untracked', !!cnUn && cnUn.untracked === true)
  check('G2 删除中文未跟踪文件真的删掉',
    g.revertFile(path.join(repo, 'bible/新章节.md'), true).ok === true
    && !fs.existsSync(path.join(repo, 'bible/新章节.md')))
  check('G2 不存在的路径点删除必须失败（rmSync force 吞 ENOENT 的假成功已修）',
    g.revertFile(path.join(repo, 'bible/根本不存在的文件.md'), true).ok === false)
  check('G2 目录不得被当文件「删除成功」',
    g.revertFile(path.join(repo, '新作品'), true).ok === false
    && fs.existsSync(path.join(repo, '新作品', 'project.md')))

  /* ── G3：重命名靠 XY 状态位，不再在路径里找 ' -> ' ─────────────────── */
  write('bible/renamed-src.md', 'aaaaaaaa\nbbbbbbbb\ncccccccc\ndddddddd\n')
  sh(['add', '--', 'bible/renamed-src.md'])
  sh(['commit', '-q', '-m', 'add rename source'])
  sh(['mv', 'bible/renamed-src.md', 'bible/renamed-dst.md'])
  const rn = g.changes().files.find((f) => f.path === 'bible/renamed-dst.md')
  check('G3 重命名解析出新路径与原路径', !!rn && rn.origPath === 'bible/renamed-src.md' && rn.status[0] === 'R',
    rn ? `${rn.status} orig=${rn.origPath}` : 'missing')
  // 兼容分支（非 -z 文本）：非重命名行里的 ' -> ' 不得被当成重命名切分
  const legacy = g.parseStatus(' M draft/a -> b.md\n')
  check('G3 兼容分支：非重命名行路径含 " -> " 不被切分',
    legacy.length === 1 && legacy[0].path === 'draft/a -> b.md', JSON.stringify(legacy))
  const legacyR = g.parseStatus('R  draft/old.md -> draft/new.md\n')
  check('G3 兼容分支：R 状态位仍按重命名取新路径与原路径',
    legacyR.length === 1 && legacyR[0].path === 'draft/new.md' && legacyR[0].origPath === 'draft/old.md', JSON.stringify(legacyR))
  check('G1 兼容分支：八进制引号路径能反转义',
    g.unquoteGitPath('"bible/\\344\\270\\226.md"') === 'bible/世.md',
    g.unquoteGitPath('"bible/\\344\\270\\226.md"'))
} catch (err) {
  check('测试自身未抛异常', false, err && err.stack ? err.stack : String(err))
} finally {
  try {
    fs.rmSync(repo, { recursive: true, force: true })
  } catch {}
}

console.log(failures.length ? 'GIT_REVIEW_FAIL ' + failures.join(' | ') : 'GIT_REVIEW_OK')
process.exit(failures.length === 0 ? 0 : 1)
