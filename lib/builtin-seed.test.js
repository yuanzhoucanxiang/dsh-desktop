...
const seedNm = path.join(seedDir, 'packages')
...
try {
  ...
  // ── 修复场景（0.1.44）：stamp 已写后，bundles 名单里的包缺失/悬空 → 修复；用户卸载 → 不顶回 ──
  // 场景 1：plugin-a 在 bundles 里，但 node_modules 条目整体缺失（悬空被清掉后的现场）
  fs.rmSync(path.join(profileNm, 'plugin-a'), { recursive: true, force: true })
  check('修复：bundles 内 + 包缺失 → needsSeed 真（stamp 已写）',
    needsSeed({ seedDir, profileDir, stampPath, quarantineNames: [] }) === true)
  {
    const r = runSeed({ seedDir, profileDir, stampPath: null })
    check('修复：runSeed 把真包复制回来（stage 版本 1.2.3）',
      r.ok && fs.existsSync(path.join(profileNm, 'plugin-a', 'package.json')) &&
      JSON.parse(fs.readFileSync(path.join(profileNm, 'plugin-a', 'package.json'), 'utf8')).version === '1.2.3')
  }

  // 场景 2：被隔离摘除（不在 bundles、在隔离记录）+ 文件缺失 → 也修（隔离不是用户意愿）
  {
    const mf = JSON.parse(fs.readFileSync(path.join(profileDir, 'package.json'), 'utf8'))
    mf.dsh.profile.bundles = mf.dsh.profile.bundles.filter((x) => x !== 'plugin-a')
    fs.writeFileSync(path.join(profileDir, 'package.json'), JSON.stringify(mf, null, 2))
    fs.rmSync(path.join(profileNm, 'plugin-a'), { recursive: true, force: true })
  }
  check('修复：不在 bundles 但在隔离记录 → needsSeed 真',
    needsSeed({ seedDir, profileDir, stampPath, quarantineNames: ['plugin-a'] }) === true)
  check('修复：不在 bundles 也不在隔离记录 → needsSeed 假（用户卸载，不顶回）',
    needsSeed({ seedDir, profileDir, stampPath, quarantineNames: [] }) === false)
  {
    const r = runSeed({ seedDir, profileDir, stampPath: null })
    check('修复：隔离场景真包回 profile 且 bundles 重新登记（bootable 闸放行）',
      r.ok && fs.existsSync(path.join(profileNm, 'plugin-a', 'package.json')) &&
      JSON.parse(fs.readFileSync(path.join(profileDir, 'package.json'), 'utf8')).dsh.profile.bundles.includes('plugin-a'))
  }

  // 场景 3：条目在但坏了（目录缺 package.json，半截删除）→ 视为缺失，替换为完整包
  {
    const mf = JSON.parse(fs.readFileSync(path.join(profileDir, 'package.json'), 'utf8'))
    if (!mf.dsh.profile.bundles.includes('plugin-a')) mf.dsh.profile.bundles.push('plugin-a')
    fs.writeFileSync(path.join(profileDir, 'package.json'), JSON.stringify(mf, null, 2))
    fs.rmSync(path.join(profileNm, 'plugin-a'), { recursive: true, force: true })
    fs.mkdirSync(path.join(profileNm, 'plugin-a'), { recursive: true })
    fs.writeFileSync(path.join(profileNm, 'plugin-a', '残骸.txt'), '半截删除')
  }
  check('修复：坏条目（缺 package.json）→ needsSeed 真',
    needsSeed({ seedDir, profileDir, stampPath, quarantineNames: [] }) === true)
  {
    runSeed({ seedDir, profileDir, stampPath: null })
    check('修复：坏条目被完整包替换，残骸清除',
      fs.existsSync(path.join(profileNm, 'plugin-a', 'package.json')) &&
      !fs.existsSync(path.join(profileNm, 'plugin-a', '残骸.txt')))
  }
