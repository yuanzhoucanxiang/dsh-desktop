...
const { readJsonSafe } = require('./atomic-file')

/**
 * 内核自带 bundle：由内核从运行时目录解析，不在 profile 的 node_modules 里，
 * 体检与契约验收都要把它们排除（否则会误报"不可加载"）。放这里作为单一来源，
 * 外壳（main.js）、体检、契约验收套件共用，避免三处各写一份而漂移。
 */
...
function inspectProfile(opts) {
  ...
  const manifest = readJsonStripBom(manifestPath)
  if (manifest === null) {
    report.error = `无法读取 profile 清单：${manifestPath}`
    return report
  }
...
/**
 * 判断单个 bundle 当前是否"能安全加载"——启动前体检与恢复隔离前的共用闸门。
 * 与 inspectProfile 的按 item 体检互补：本函数不依赖该 bundle 是否已在 bundles
 * 列表里（恢复隔离时它不在列表，inspectProfile 会漏判 no-dsh-bundle）。
 * 判定条件对齐内核解析路径：node_modules 条目可解析 + package.json 可读 +
 * dsh.bundle.patch 声明存在；link: 依赖还要声明目标真实存在。
 * @param {string} name bundle 包名
 * @param {object} opts { manifestPath, nodeModulesDir }
 * @returns {{ok:boolean, reason?:string}}
 */
function bundleBootable(name, opts) {
  const o = opts || {}
  if (!o.manifestPath) return { ok: false, reason: '缺少 manifestPath' }
  const nodeModulesDir = o.nodeModulesDir || path.join(path.dirname(o.manifestPath), 'node_modules')
  const entry = path.join(nodeModulesDir, name)
  // statSync 穿透 junction/symlink：悬空链接在这里表现为不可达
  const reachable = safe(() => !!fs.statSync(entry), false)
  if (!reachable) return { ok: false, reason: 'node_modules 条目缺失或悬空' }
  const pkg = safe(() => readJsonStripBom(path.join(entry, 'package.json')), null)
  if (!pkg) return { ok: false, reason: 'package.json 不可读' }
  if (!(pkg.dsh && pkg.dsh.bundle && pkg.dsh.bundle.patch)) return { ok: false, reason: '缺 dsh.bundle.patch 声明' }
  const manifest = safe(() => readJsonStripBom(o.manifestPath), null)
  const spec = manifest && manifest.dependencies && manifest.dependencies[name]
  if (spec && /^link:/i.test(spec)) {
    const target = spec.replace(/^link:/i, '')
    if (!safe(() => !!fs.statSync(target), false)) return { ok: false, reason: `link 目标不存在：${target}` }
  }
  return { ok: true }
}
...
module.exports = { inspectProfile, bundleBootable, compareVersions, PROBLEM_LABELS, readJsonStripBom, CORE_BUNDLES }
