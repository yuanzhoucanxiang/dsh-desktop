...
/**
 * S4：允许写进 YAML 的 entry id 形态（白名单）。
 *
 * `setEntryDisabled` 把 id **原样插值**进 `- id: ${id}`，而这个 id 来自插件自己的
 * dsh.bundle.patch（`entryIdsFromBundlePatch` 只做去引号）。实测：一个含换行的 id
 * 能把未经解析器审查的额外条目写进 profile 补丁（而这份文件决定内核加载什么），
 * 也能写出解析不了的形状把整个开关功能锁死——后者正好绕过本文件开头承诺的
 * 「解析不了的文件一律拒绝写入（宁可不给开关也不毁手工编辑）」。
 * 不是权限提升（host 侧插件本来就有 Node），但该守的输入校验不能省：不符就拒写，绝不猜。
 */
...
function setEntryDisabled(file, id, disable) {
  if (typeof id !== 'string' || !SAFE_ENTRY_ID.test(id)) {
    return {
      ok: false,
      error: `entry id 含不安全字符（换行/引号/空白等），已拒绝写入补丁：${JSON.stringify(String(id ?? '').slice(0, 80))}`,
      changed: false,
    }
  }
  let text = null
