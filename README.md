...
两套皮肤共用同一套 DOM 与同一个进度数值，只是表达方式不同：
...
- **进度不撒谎**：弧长/线宽由真实相位里程碑驱动（拉起 → 等待 → 就绪），里程碑之间渐近爬升；
...
> **皮肤只作用于外壳自己拥有的界面**（启动画面、窗口底色、预览窗口）。内核页面与注入的
> 「修改审阅」侧边栏一律跟随**内核自己的设计令牌**（`--dsw-alias-*`），外壳不去染色 ——
> 这也是「内核零修改」的一部分：内核换主题时侧边栏自动跟着变，不会打架。

海景皮肤的三条硬约束写进了回归测试（`splash-check.js`），改样式时不会悄悄跑偏：
地平线必须落在画面中线（±1px）、天与海必须等高（±1px）、画面必须近乎单色（色偏 ≤14）。
...
npm run splash-check                          :: 两套皮肤的布局/进度/概念/像素/握手回归，163 项断言
