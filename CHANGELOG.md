...
- **内核升级 0.1.1-rc.1 → 0.1.7-rc.2**：跟上官方 6 个 rc 周期（渲染门禁、契约探针、插件实测全绿）。
  运行时改用官方 npm 源装配（镜像缺子包会装出残树）。— 署名：ZCode
- **新加载架构（dsh-app 协议）**：渲染进程只认 `dsh-app://app`，前端构建产物由外壳本地直出、
  其余请求代持内核 cookie 转发（与官方桌面端同构）。内核随机端口与一次性 token 不再进入渲染进程；
  WebSocket 目标自动改写到内核（第三方插件的终端/文件监听、内核自身 remote.mux 都依赖这层）。
  `DSH_DESKTOP_SCHEME=0` 可整条回退旧直连方式。— 署名：ZCode
- **profile 内核包链接迁移**：每次拉内核前自动把 profile 里的 `@deepseek-ai/*` 条目对齐到本轮
  运行时（幂等）——既存 profile 升级内核不再出现「旧代码喂给新内核」的启动失败。— 署名：ZCode
- **内置插件升级**：dsh-better-sidebar 0.15.2 → 0.21.1（0.15.2 在新内核上无法加载，逐版本实测选型）；
  @dsh-local/palis-theme-panel 0.5.13 → 0.5.14（内核 0.1.7 适配，主题设置跨升级保留）。
  升级环境首启会自动补齐。— 署名：ZCode
- **坏插件识别跟上新内核文案**：0.1.7 的加载失败日志换了格式，隔离安全网同步更新（新旧两种都认），
  配单测。— 署名：ZCode
- 修复 `shell-hardening-test` 的 S6 断言仍引用已移除审阅侧栏文案的问题，改为钉住休眠通道契约。— 署名：kimi

## [0.1.44] - 2026-09-25

- 移除外壳自带的「审阅」侧边栏界面（原右侧栏的修改审阅/版本回退面板），审阅体验改由内置插件 dsh-better-sidebar 提供（新装/升级会自动带上并登记）。窗口标题与托盘的「运行中」提示由后台事件流驱动，不受影响。— 署名：ox-alpha
- 修复内置插件在部分老 profile 上缺失的问题（种子一次性导致真包未落地）：升级时会按安装包内置清单补齐缺失的插件包（dsh-better-sidebar / dsh-pet / 主题面板等），用户已卸载或禁用的仍然不会被顶回来。— 署名：ox-alpha
## [0.1.43] - 2026-09-25
