...
### 新增：插件体检面板（主界面左下角「⚙ 插件」/ 托盘「插件与体检…」）

- **一键体检**（纯只读，不改任何文件）：逐项检查 profile 插件的安装完整性、
  bundles 启用一致性、链接是否悬空、`dsh.bundle` 声明、隔离名单——覆盖
  「侧边栏静默消失」「pnpm 跨盘链接悬空」「缺声明内核即崩」三类真实事故。
- **更新检测**：npm 来源插件比对 registry 最新版（只查不装）；本地链接插件显示
  「本地开发版」。一键更新暂缓（等链接自修复就位，避免 pnpm 打断本地链接）。
- **入口**：主界面左下角「⚙ 插件」（体检异常时红点亮起）、托盘菜单常驻入口 +
  异常警示入口；应用启动时自动静默体检一次。
- **隔离恢复可视化**：面板内展示被隔离插件及原因，可一键恢复并重启内核。
