...
function writeKernelPatch(mode) {
  try {
    ...
    writeFileAtomic(patchFile, yaml) // 原子替换：内核读到的永远是完整补丁，不会半截
...
/** 读 JSON 并剥掉可能的 UTF-8 BOM（PS5.1 `Set-Content -Encoding UTF8` 会写 BOM，
 *  内核与本应用对带 BOM 的 manifest 都会 JSON.parse 失败）。统一走 lib/atomic-file
 *  的 readJsonSafe：剥 BOM + 解析失败（半截/非法）返回 null 而非抛。 */
 ...
 *  体检 findNonBootableBundles 确定性兜住，不再等内核崩了刮日志逐条加正则。 */
...
/** 启动前体检：对 profile 启用列表里的每个 bundle 做可加载性校验（bundleBootable），
 *  返回当前**会直接炸内核**的 bundle 名（含原因）。确定性判定，不依赖错误文案。 */
function findNonBootableBundles() {
  const file = profileWebManifest()
  let manifest
  try {
    manifest = readJsonStripBom(file)
  } catch (err) {
    log(`pre-flight: cannot read profile manifest (${err.message})`)
    return []
  }
  ...
  for (const name of bundles) {
    if (name.includes(':') || CORE_BUNDLES.has(name)) continue // 内核自带条目不检
    const b = bundleBootable(name, { manifestPath: file, nodeModulesDir })
    if (!b.ok) bad.push({ name, reason: b.reason })
  }
  return bad
}
...
function quarantineBundles(names, reason) {
  ...
  try {
    writeJsonAtomic(file, manifest) // 原子替换：重写 manifest 时崩溃/被杀不产生半截文件
  } catch (err) {
  ...
  try { writeJsonAtomic(quarantineFile(), list) } catch {}
...
/**
 * 一键恢复：把隔离过的 bundle 加回 profile。**只恢复当前能安全加载的**——恢复前
 * 逐个跑 bundleBootable（node_modules 可解析 + dsh.bundle 声明 + link 目标存在），
 * 仍坏的不加回（保持隔离并说明原因），避免"恢复→启动又崩→再隔离"的循环。
 * 恢复成功后重启内核。
 */
function restoreQuarantinedBundles() {
  ...
  try {
    ...
    let skipped = 0
    for (const e of list) {
      if (!deps[e.name]) { skipped++; log(`restore skip ${e.name}: 依赖未声明`); continue }
      if (bundles.includes(e.name)) { restored++; continue } // 已在启用列表，无需再加
      const b = bundleBootable(e.name, { manifestPath: file, nodeModulesDir })
      if (!b.ok) { skipped++; log(`restore skip ${e.name}: ${b.reason}`); continue }
      bundles.push(e.name)
      restored++
    }
    manifest.dsh.profile.bundles = bundles
    if (restored > 0) writeJsonAtomic(file, manifest) // 原子替换
    // 只清掉已恢复/已回列表的隔离记录；仍坏的保持隔离，免得下次启动又触发
    const remaining = list.filter((e) => !bundles.includes(e.name))
    if (remaining.length) writeJsonAtomic(quarantineFile(), remaining)
    else try { fs.rmSync(quarantineFile(), { force: true }) } catch {}
    log(`restored ${restored} quarantined bundle(s)${skipped ? `, ${skipped} still broken kept quarantined` : ''}`)
    refreshMenus()
    if (restored > 0) restartKernel()
  } catch (err) {
...
/**
 * startKernel + waitReady，带启动恢复链：
 *  ① 启动前体检（确定性）：启用列表里当前无法加载的 bundle（缺 dsh.bundle/链接悬空/
 *     缺条目）直接隔离摘除，不启动内核，避免"崩了再猜"——这是主防线。
 *  ② 运行期安全网：插件代码 import 时抛错（静态体检看不见）→ 刮日志 detectBrokenBundles。
 *  ③ 补丁降级 full→bridge→none（仅初始启动启用；内核重启只做前两级，避免慢启动被误降级）。
 * 任一轮成功即返回；全部失败向上抛（进入启动失败画面 / 内核状态提示）。
 */
async function startKernelUntilReady({ degradePatch = false } = {}) {
  for (let round = 0; ; round++) {
    // ① 启动前体检：把当前会炸内核的 bundle 确定性摘掉，摘干净再启动
    const nonBootable = findNonBootableBundles()
    if (nonBootable.length) {
      const known = new Set(readQuarantine().map((e) => e.name))
      const fresh = nonBootable.map((b) => b.name).filter((n) => !known.has(n))
      const removed = quarantineBundles(fresh, nonBootable.find((b) => b.name === fresh[0])?.reason || 'pre-flight')
      if (removed.length) {
        notifyQuarantine(removed)
        continue // 摘掉后重新体检，保证连环坏插件一次清干净再启动
      }
    }
    try {
      await startKernel()
      ...
      // ② 运行期安全网：import 时抛错（体检抓不到）→ 按日志点名隔离
...
async function startKernel() {
  ...
  // 外壳自己用原生窗口承载内核页面，必须带 --no-open：内核 web-startup 默认会用
  // 系统默认浏览器打开 Web UI（抢焦点、与桌面窗口重复），浏览器交接永远不该发生。
  const baseArgs = ['--profile', 'web', ...patchArgs, '--port', String(state.port), '--no-open']
