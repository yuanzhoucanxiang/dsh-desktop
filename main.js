...
/**
 * 生成审阅桥补丁文件（写入 userData），供内核 --patch 注入：
 *   - 插件经 file:// URL 引用（Node ESM 在 Windows 不接受裸绝对路径）
 *   - 事件流输出到 userData/review-events.ndjson，每次内核启动重置
 * 插件文件缺失时返回 ''（内核不带补丁启动，审阅功能降级为仅 git 视图）。
 */
function writeReviewPatch() {
  try {
    ...
    const patchFile = path.join(dir, 'review-bridge.patch.yml')
    const url = pathToFileURL(pluginFile).href
    const yaml = [
      '# generated by the desktop shell',
      '- insert:',
      '    - id: review-bridge',
      `      name: '${url}'`,
      '      config:',
    ...
    log(`review bridge patch: ${patchFile}`)
    return patchFile
  } catch (err) {
    log(`writeReviewPatch failed: ${err.message}`)
    return ''
  }
}
