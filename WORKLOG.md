...
全局热键静默失效已排查并修复（用户报“回来 debug”）。**症结不是注册逻辑，而是静默与无自救出口**：正式版 kernel.log 里热键成功 17 次 / 被拒 92 次，最后一次成功是 09-15，Win32 `RegisterHotKey` 探测证实 `Ctrl+Alt+D` 真的被其他进程占着（`Win32Error=1409`）——但这件事唯一的信号是日志里一行字，且托盘只能开关不能改键。现修：注册结果进 `state.hotkey` 并经 get-state 下发、托盘标签显示“· 被其他程序占用”、重试到头发系统通知（A）；新增 `shell:set-global-hotkey`（**只给设置窗口**）+ 「通知」页热键卡片（B）；1s/3s/8s 退避重试（C）；新增 `lib/hotkey.js` 白名单拦住裸键与 `Control+字母`（Electron 不会替你挡的自我伤害：前者劫持键盘、后者让全系统无法复制/保存/撤销）。验收：`hotkey-test` **68 项**、`shell-hardening-test` 42→47、`verify-ipc-authz` 11→**23 项**（dev + 装机版均 `IPC_AUTHZ_OK`）。**这轮补上了一个一直存在的验证缺口**：以前只测“非设置窗口被拒”，从没测“设置窗口被放行”——万一 `isSettingsSender` 恒为假，S1 就是静默的功能损坏而断言全绿；现在探针会真开设置窗口验证（实测状态行渲染、改键成功、三类非法输入各自被拒）。真包重打 asar `d2c94c55…`→`5e633510…`，8+12+9 全过。**D 未完成**：没查出哪个进程占着 Ctrl+Alt+D（需逐个关应用，要用户授权），但 6 个候选探测下来**只有 D 被占**，H/K/F1/J/Insert 全空闲——用户改键即可自救。未提交、未发布。— ox-alpha，2026-09-22
